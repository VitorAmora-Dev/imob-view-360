...
describe('PanoramicViewerComponent — zoom limitado do tour', () => {
  ...
  it('a roda amplia no máximo cinquenta por cento e não desloca a câmera', () => {
    const camera = component.viewerCamera!;
    ...
    expect(camera.zoom).toBeCloseTo(1.5, 8);
  ...
  it('a pinça amplia até cinquenta por cento', () => {
    ponteiro('pointerdown', 1, 100);
    ponteiro('pointerdown', 2, 200);
    ponteiro('pointermove', 2, 250);

    expect(component.viewerCamera!.zoom).toBeCloseTo(1.5, 8);

    ponteiro('pointerup', 2, 250);
...
describe('PanoramicViewerComponent — modo paisagem', () => {
  ...
  it('a pinça deitado amplia sem girar a câmera', async () => {
    ...
    canvas.dispatchEvent(evento('pointermove', 2, 250));

    expect(component.viewerCamera!.zoom).toBeCloseTo(1.5, 8);
    expect(component.viewerCamera!.position.distanceTo(antes)).toBeCloseTo(0, 8);

    canvas.dispatchEvent(evento('pointerup', 2, 250));
