...
export class GuidedHotspotsComponent {
  ...
  constructor() {
     ...
     * campo em `tour-wizard.model.ts`. Sem isto o assistente abre com a esfera
     * branca: o `TextureLoader` falha calado com endereço vazio e o material
     * fica sem mapa.
     *
     * O editor livre tem o mesmo effect. Ele nasceu lá, quando a etapa 2 era um
     * componente só, e não veio para cá na fusão porque a branch do rascunho é
     * anterior ao assistente e não sabia que este modo existia.
     */
    effect(() => {
      const cena = this.guided.passo()?.scene;
      if (!cena || cena.treatedImageUrl || cena.imageData) return;
      // `.catch` porque o effect reroda a cada mutação da cena — um ponto
      // marcado basta. Sem ele, falha de rede vira rejeição sem dono a cada
      // gesto.
