...
describe('GuidedHotspotsComponent', () => {
  ...
  /**
   * A foto de uma cena retomada, no modo guiado.
   *
   * O rascunho retomado devolve a cena SEM foto -- `imageData` vazio e
   * `serverPanoramaId` preenchido. Quem abre a etapa 2 e quem primeiro precisa
   * da imagem. O editor livre ja fazia isso; o assistente nao, porque a branch
   * do rascunho e anterior a ele e nao sabia que este modo existia. Sem o
   * pedido, o assistente abre com a esfera branca.
   */
  it('pede a foto ao abrir uma cena retomada', () => {
    const retomada = (id: string): WizardScene => ({
      ...cena(id),
      imageData: '',
    ...
    draft = TestBed.inject(TourDraftStore);
    draft.scenes.set([retomada('sala'), retomada('cozinha')]);
    draft.selectedSceneId.set('sala');

    // O espiao entra ANTES da primeira rodada: depois dela o effect ja teria
    // chamado a implementacao de verdade e batido no PanoramaImageCache.
    const pedido = spyOn(draft, 'garantirImagem').and.resolveTo('blob:x');

    fixture = TestBed.createComponent(GuidedHotspotsComponent);
    fixture.detectChanges();

    expect(pedido).toHaveBeenCalledWith('sala', 'treated');
  });

  it('nao pede foto para cena que ja tem imagem', () => {
    montar([cena('sala'), cena('cozinha')]);
    const pedido = spyOn(draft, 'garantirImagem');
