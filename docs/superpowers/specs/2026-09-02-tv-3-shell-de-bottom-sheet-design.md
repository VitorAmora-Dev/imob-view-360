...
| | |
|---|---|
| Inputs | `isOpen: boolean`, `titulo: string`, `subtitulo: string \| null = null`, `breakpoints: number[] = [0, 0.55]`, `initialBreakpoint: number = 0.55` |
| Output | `(fechado)` |
| Corpo | `<ng-content>` |
...
**Grabber**: o Ionic desenha o handle sozinho quando há `breakpoints`.
Nada a fazer além de não escondê-lo.
